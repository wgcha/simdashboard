...
파일은 로컬에서 생성한 뒤 2026-09-07에 아래 사전 배포판으로 게시했습니다.

https://github.com/wgcha/simdashboard/releases/tag/offline-rocky8.6-20260907-rc1

게시 소스 커밋은 `c0b77b8184c7672a7cdc0c9db2804aace00926e8`입니다. 빌드 커밋과
프로그램/설치 파일은 동일하며 미게시 GitHub Actions workflow 하나만 제외했습니다.
main은 변경하지 않았습니다. 네 첨부파일 업로드 및 설치 파일의 서버 SHA-256이
위 로컬 값과 일치함을 확인했습니다. 비공개 저장소이므로 접근 권한이 있는 GitHub
로그인이 필요합니다. 이 설치 파일은 `git pull`이 아니라 Releases의 Assets에서
받아 승인된 파일 전송 방식으로 반입합니다. 기존 작업 중인 프런트엔드 변경은
포함하지 않고 위 빌드 커밋의 깨끗한 소스로 만들었습니다.
