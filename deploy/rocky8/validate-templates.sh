...
grep -Fq 'SIMDASH_PYTHON_RUNTIME_CACHE' "${root}/bootstrap-python-runtime.sh"
grep -Fq -- '--system-certs --no-progress python install' "${root}/bootstrap-python-runtime.sh"
grep -Fq 'SIMDASH_NODE_RUNTIME_CACHE' "${root}/deploy-from-source.sh"
grep -Fq -- '--preserve-env=HTTP_PROXY,HTTPS_PROXY,NO_PROXY,http_proxy,https_proxy,no_proxy,CURL_CA_BUNDLE,SSL_CERT_FILE' "${root}/deploy-from-source.sh"
