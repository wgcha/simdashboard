...
작업 유형 관리에서 부서 업무에 사용할 세부 작업과 업무 유형을 정의하고, 의뢰 접수에서 새로 만든 활성 업무 유형을 선택할 수 있게 한다. SPDM은 의뢰 번호와 요청 내용을 제공하는 **접수 원천**이고, Altair One 공유폴더는 접수된 의뢰가 실제로 수행되는 **의뢰별 작업공간**이다. 두 경계를 분리한 뒤 부서장 수기 지시와 같은 공통 수명주기로 관리한다.
...
- SPDM 의뢰는 SPDM 의뢰 번호로 한 번만 생성한다. 관리자가 업무 유형과 담당자를 확정하면 Altair One 의뢰별 작업폴더를 생성하거나 기존 폴더를 검증한 뒤 연결하고 담당자 수령 대기로 전환한다.
- Altair One 작업폴더 변화는 새 의뢰를 만들지 않고 동일 의뢰의 작업공간 변경 이력으로 기록한다. 진행 상태는 유지하며 앱 내 배지와 확인 이력을 제공한다.
- Altair One 작업공간은 프로젝트/의뢰 폴더 레벨, 자동 주기 감지와 관리자 수동 재감지를 지원한다. SPDM 원천을 공유폴더 스캔으로 추정하지 않는다.
...
| 의뢰 발생 | Occurred | SPDM 접수 또는 부서장 등록으로 의뢰 레코드가 만들어진 상태. |
| SPDM 원천 | Request Origin | SPDM 의뢰 번호·요청 내용·접수 시각의 신뢰할 수 있는 원천. 의뢰 작업 파일을 보관하는 폴더가 아니다. |
| Altair One 작업공간 | Request Workspace | 하나의 의뢰에 연결된 Altair One 공유폴더. 실무자가 입력·중간 산출물·결과 후보를 정리하는 장소다. |
...
### 2.3 SPDM 접수와 Altair One 작업공간 흐름

1. SPDM API·이벤트 또는 승인된 가져오기로 의뢰 번호와 요청 내용을 수신해 `SPDM` 출처 의뢰를 `OCCURRED`로 한 번만 생성한다.
2. 프로젝트 관리자가 접수 목록에서 업무 유형·담당자·기한을 확정한다.
3. 사내 Altair One 연동 방식과 권한에 따라 서버가 승인된 공유 루트 아래에 의뢰별 작업폴더를 생성하거나 기존 폴더를 검증한 뒤 해당 경로를 의뢰에 연결한다. 생성·검증·binding 중 하나라도 실패하면 수령 대기로 전환하지 않는다.
4. 작업계획을 생성하고 의뢰를 `PENDING_ACCEPTANCE`로 전환한다.
5. 담당자가 수령한 뒤 Altair One 작업폴더에서 수기 지시와 동일한 수행 흐름을 따른다.
6. 같은 작업폴더의 구조 또는 파일 메타데이터가 바뀌면 동일 의뢰에 작업공간 변경 이벤트를 추가하고 미확인 배지를 표시한다. 접수·수행·완료 상태는 자동으로 되돌리지 않는다.
7. 수행 결과는 작업폴더에서 준비한 뒤 canonical 결과 bundle로 발행한다. 발행마다 새 불변 Run(v1, v2, ...)을 적재하며 사용자는 대시보드에서 표시할 Run을 선택한다.

### 2.4 상태 규칙

| 의뢰 상태 | 진입 조건 | 허용 동작 |
|---|---|---|
| `OCCURRED` | SPDM 의뢰가 접수되어 원천 정보가 저장됨 | 관리자의 유형·담당자 확정, 중복/오탐 제외 |
...
| `COMPLETED` | 필수 세부 작업 모두 완료 | 결과 버전 열람, Altair One 작업공간 후속 변경 확인 |
...
- Altair One 작업공간 변경 이벤트는 의뢰 수명주기와 독립적이다. 완료 후 변경도 기록하되 의뢰를 재개하지 않는다.
...
`request_origins`는 SPDM 접수 원천만 표현한다. Altair One 작업폴더의 경로·생성 시각·변경 fingerprint는 별도 binding에 저장하며, `source_reference`에 작업공간 경로를 섞지 않는다.

### 3.3 Altair One 작업공간 모델

다음 테이블을 추가한다.

```text
altair_one_workspace_configs
- id, project_id, display_name
- workspace_root_ref: 사내 서버 설정에 등록된 허용 루트의 opaque 참조
- root_alias: 화면·감사로그에만 쓰는 허용 루트 별칭(클라이언트가 절대 경로를 저장하지 않음)
...
request_workspace_bindings
- request_id PK/FK
- workspace_config_id FK
- provider: ALTAIR_ONE
- normalized_relative_path
- workspace_key UNIQUE per workspace config
- created_at, created_by
- last_fingerprint
- first_detected_at, last_seen_at

request_workspace_change_events
...
- `request_workspace_bindings`로 이미 연결된 의뢰 작업폴더 하나를 하나의 작업공간으로 본다. 스캔이 의뢰를 새로 만들지 않는다.
- fingerprint는 Altair One 의뢰 작업폴더 아래의 정규화된 상대 경로, 항목 종류, 파일 크기, 수정 시각을 정렬해 계산한다. 파일 본문은 읽거나 저장하지 않는다.
- 심볼릭 링크와 감시 루트 밖으로 해석되는 경로는 따라가지 않는다.
- 임시 파일 패턴(`~*`, `*.tmp`, `.DS_Store`)은 기본 제외하며 설정에서 추가 제외 패턴을 허용한다.
- 같은 SPDM external key는 반복 가져오기해도 의뢰를 중복 생성하지 않고, 같은 workspace key는 반복 스캔해도 binding을 중복 생성하지 않는다.
- 스캔 도중 접근 실패한 폴더는 기존 항목 삭제로 판정하지 않고 scan 오류로 기록한다.
- 변경 경로 목록은 상한을 두고 초과 시 `truncated=true`로 기록한다.

자동 감지는 애플리케이션 lifespan의 단일 주기 작업으로 실행하고, 현재 단일 Uvicorn 프로세스 배포를 기준으로 한다. PostgreSQL 다중 프로세스 배포 시에는 advisory lock으로 동일 작업공간 설정의 중복 스캔을 막는다. `ALTAIR_ONE_ALLOWED_ROOTS` 환경변수에 등록된 canonical 루트 아래 경로만 작업공간 설정으로 저장할 수 있게 한다. SPDM 원천 수신은 별도의 API/event adapter이며 이 파일 감시 권한을 요구하지 않는다.

클라이언트는 `root_path`나 UNC/NFS 절대 경로를 보내거나 저장할 수 없다. 작업공간
설정은 서버가 가진 `workspace_root_ref`/`root_alias` 목록에서 선택하며,
`request_workspace_bindings.normalized_relative_path`만 결합해 실제 경로를 서버에서
해결한다.
...
GET  /api/projects/{project_id}/altair-one-workspace-configs
POST /api/projects/{project_id}/altair-one-workspace-configs
PUT  /api/projects/{project_id}/altair-one-workspace-configs/{config_id}
POST /api/projects/{project_id}/altair-one-workspace-configs/{config_id}/scan
GET  /api/projects/{project_id}/spdm-intake
POST /api/integrations/spdm/requests
POST /api/requests/{request_id}/assign-work-plan
GET  /api/requests/{request_id}/workspace-changes
POST /api/requests/{request_id}/workspace-changes/{change_id}/acknowledge
```
...
- SPDM 의뢰의 `assign-work-plan`은 활성 업무 유형, 프로젝트 범위, 활성 담당자 멤버십과 Altair One 작업폴더의 생성 또는 검증 후 binding을 확인한 뒤 배정·작업계획을 원자적으로 생성한다.
...
- 목록과 상세 응답에 `lifecycle_status`, `accepted_at`, `source`, `workspace`, `unacknowledged_workspace_change_count`를 포함한다.
...
- Altair One 작업공간 설정·수동 재감지·SPDM 접수 의뢰 배정은 `project.work_type.manage`가 필요하다.
- 의뢰 발생/배정은 `request.create`, 담당자 수령은 배정자 본인 또는 `work.execute_any`, 변경 확인은 프로젝트 멤버 중 의뢰 열람 권한 보유자에게 허용한다.
- 모든 유형 생성, SPDM 수신, Altair One 작업공간 설정·생성, 수동 스캔, 의뢰 배정·수령, 변경 확인을 감사로그에 남긴다.
...
2. `SPDM 접수 대기`
   - `OCCURRED` 의뢰의 SPDM 의뢰 번호, 접수 시각, 요청 요약을 표시한다. 연결된 Altair One 작업폴더는 별도 필드로 표시한다.
...
- Altair One 작업공간 변경이 있으면 의뢰 카드와 상세에 미확인 개수 배지를 표시하고, 변경 경로 요약과 감지 시각을 본 뒤 `변경 확인`할 수 있게 한다.
- 변경 확인은 알림만 해제하며 업무 상태와 진행률을 바꾸지 않는다.

## 5. 마이그레이션과 구현 순서

1. 새 Alembic migration에 카탈로그 scope, SPDM 접수 원천·수령 이력, Altair One 작업공간 binding/변경 테이블과 인덱스·제약을 추가하고 `schema.sql` 및 로컬 초기화 DDL을 동기화한다.
...
5. SPDM intake adapter와 Altair One 작업폴더 생성 또는 검증·binding·스캔 서비스, 시작/종료 lifecycle, 수동 스캔, fingerprint 및 변경 확인 API를 구현한다.
...
- 동일 SPDM 의뢰 번호 반복 수신은 의뢰를 중복 생성하지 않는다.
- 신규 SPDM 의뢰는 `OCCURRED`가 되고 관리자가 유형·담당자를 배정하면 Altair One 작업폴더 생성 또는 검증 후 binding과 함께 `PENDING_ACCEPTANCE`가 된다.
...
- SPDM 접수 대기 항목에 유형과 담당자를 배정해 Altair One 작업폴더가 생성되거나 검증·연결된 뒤, 담당자가 수령해 수행 화면으로 진입한다.
...
- SPDM 신규 의뢰가 한 번만 접수되고, 연결된 Altair One 작업폴더가 생성 또는 검증 후 한 번만 binding된다.
- 수기/SPDM 의뢰 모두 담당자 수령 전에는 수행을 시작할 수 없다.
- 수행자는 각 Custom 세부 작업의 설명을 보고 시작·진행·완료할 수 있다.
- Altair One 작업공간 변경은 동일 의뢰에 추적되고 확인 이력이 남으며 기존 진행 상태를 훼손하지 않는다.
...
- SPDM API/event adapter의 실제 인증·네트워크 연결은 사내 환경 구현 범위다. 로컬에서는 fixture adapter로 계약과 idempotency를 검증한다.
- 파일 내용 hash, 파일 본문 저장, 결과 자동 import는 제외한다.
- Custom 세부 작업 체크리스트, 승인 단계, 반려/취소/재오픈 상태는 후속 범위로 둔다.
- Altair One 작업공간 변경은 자동 재접수 또는 신규 의뢰 생성을 유발하지 않는다.
