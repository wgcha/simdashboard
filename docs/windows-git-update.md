...
`log/` 아래에서는 `.md` 문서만 변경 검사 대상으로 유지한다(하위 폴더·대문자 `.MD` 포함). 그 밖의 미추적 로그·텍스트·JSON 파일은 Git에서 제외하고 PC에 그대로 보존한다. `log/work-log.md`처럼 Git이 관리하는 문서의 수정은 계속 검사한다. 이미 Git에 추적된 파일은 확장자만으로 검사를 우회하지 않는다.

`docs/` 아래의 기존 문서 수정·삭제와 새 문서는 업데이트기의 사전 변경 검사에서 제외한다. 문서만 수정된 상태에서도 다른 소스의 업데이트를 진행할 수 있다. 문서 추적 자체를 해제하거나 자동 stash/reset하지 않으며, 일반 `git status`에서는 계속 보일 수 있다. 원격 업데이트가 같은 문서와 충돌하거나 새 로컬 문서를 덮어쓰려는 경우에는 Git의 보호 검사가 업데이트를 중단하므로 해당 문서를 별도 보존한 후 충돌을 해결한다. `docs/` 밖의 소스와 `log/`의 Markdown 문서 검사는 유지한다.
