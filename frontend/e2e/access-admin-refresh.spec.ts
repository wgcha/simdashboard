...
test('권한별 배정과 사용자관리 새로고침은 도우미 없이 동작하고 조회 실패 후 복구한다', async ({ page }) => {
  ...
  await userRow.getByRole('button', { name: '프로젝트에 추가', exact: true }).click()
  ...
  await generalGroup.locator('article').filter({ hasText: username }).getByRole('combobox').selectOption('power')
  ...
  await powerGroup.locator('article').filter({ hasText: username }).getByRole('combobox').selectOption('admin')
