...
for (const extension of ['csv', 'json']) {
  test(`issue ${extension} sample: item editing, widgets, atomic save, reopen and Run`, async ({ page }, testInfo) => {
    ...
    const picker = componentEditor.getByLabel('위젯 결과 항목', { exact: true })
    ...
    await expect(preview.locator('.semantic-result-kpi strong')).toHaveText(['-311.643', '-159.909'])
    ...
    expect(saved.template.definition.widgets[2].type).toBe('kpi')
    ...
    await expect(editors.last().getByLabel('위젯 종류')).toHaveValue('kpi')
