...
test('첫 화면은 독립 결과 대시보드이며 실제 최신 Run 집계와 일치한다', async ({ page }) => {
  ...
  await expect(dashboard.getByRole('button', { name: '검토 필요', exact: true }).locator('b')).toHaveText(String(data.records.filter((record) => record.run_id && record.result_count > 0 && record.verdict === 'FAIL').length))
  ...
  await dashboard.getByRole('button', { name: '검토 필요', exact: true }).click()
  await expect(dashboard.getByRole('button', { name: '검토 필요', exact: true })).toHaveAttribute('aria-pressed', 'true')
...
test('대기 의뢰로 이동 중에는 이전 의뢰의 작업을 실행할 수 없다', async ({ page }) => {
  ...
  await expect(workbench.getByRole('heading', { name: '워크플로: 진행·차단·대기', exact: true })).toBeVisible()
