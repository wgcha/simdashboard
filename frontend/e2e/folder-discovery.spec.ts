...
test('folder discovery creates real project, request and load case then keeps them on reapply', async ({ page }, testInfo) => {
  ...
  await catalogEditor.locator(':scope > summary').click()
  ...
  await expect(screen.getByRole('status').filter({ hasText: '프로젝트 1' })).toBeVisible()
  ...
  await screen.getByRole('button', { name: '검토한 업무 생성 적용', exact: true }).click()
  await expect(screen.getByRole('status').filter({ hasText: '제외 2개' })).toBeVisible()
