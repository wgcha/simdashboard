...
import { ProjectSetupState } from './ProjectSetupState'
...
type Props = {
  ...
  onSelectAnalysisRun: (id: string) => void
...
export function RequestWorkspaceRoute({
  activeDashboardId, activeView, analysisPages, analysisRuns, analysisRunsLoading, analysisRunChanging, analysisRunError, selectedAnalysisRunId, canEdit, canManagePages, chassisThreshold, dashboard,
  canCreateRequest = false, editMode, loadCases, onAddStep, onBeginEditing, onCancelEditing, onConfigureWidget,
  onComparisonContext, onDashboardLayoutChange, onDeleteStep, onLayoutChange, onMoveStep, onOpenAnalysis, onOpenAssistant,
  onOpenIntake, onOpenPageManager, onRemoveWidget, onSaveChassisThreshold, onSaveOpenCellThreshold,
  onSelectLoadCase, onSelectAnalysisRun, onSelectProject, onSelectRequest, onStepChange, onSwitchAnalysisPage,
  onSwitchView, onUpdateWorkflowLayout, openCellThreshold, overview, projectWorkflows, projects,
  projectName, projectSetup = false, requests, resetWorkflowDashboardLayout, selectedEdges, selectedLoadCaseId, selectedProjectId,
  ...
  const analysisStatus = (page: DashboardPageSummary) => page.page.analysis_key === 'open_cell' ? overview.analysis_verdicts.open_cell : page.page.analysis_key === 'chassis_rear' ? overview.analysis_verdicts.chassis_rear : page.page.analysis_key === 'run_comparison' ? 'SYSTEM' : page.page.status.toUpperCase()
  ...
  if (projectSetup) return <>
    <section className="content-head">
      <div><div className="eyebrow"><span>PROJECT SETUP</span><span>•</span><span>REQUEST INTAKE</span></div><h1>{projectName ?? '선택한 프로젝트'}</h1><p>프로젝트가 선택되었습니다. 첫 의뢰를 접수하면 작업공간이 열립니다.</p></div>
      <div className="context-selectors"><label><span>프로젝트</span><SearchableSelect ariaLabel="프로젝트 선택" items={projects} kind="project" value={selectedProjectId} onChange={(value) => void onSelectProject(value)} placeholder="프로젝트 선택" /></label><label><span>의뢰</span><SearchableSelect ariaLabel="의뢰 선택" items={[]} kind="request" value="" disabled placeholder="아직 없음" onChange={() => undefined} /></label><label><span>하중 경우</span><SearchableSelect ariaLabel="하중 경우 선택" items={[]} kind="load_case" value="" disabled placeholder="의뢰 접수 후 설정" onChange={() => undefined} /></label></div>
    </section>
    <ProjectSetupState projectName={projectName ?? '선택한 프로젝트'} canCreateRequest={canCreateRequest} onOpenIntake={onOpenIntake ?? (() => undefined)} />
  </>
  ...
  return <>
    <section className="content-head">
      ...
      <div className="context-selectors"><label><span>프로젝트</span><SearchableSelect ariaLabel="프로젝트 선택" items={projects} kind="project" value={selectedProjectId} onChange={(value) => void onSelectProject(value)} placeholder="프로젝트 선택" /></label><label><span>의뢰</span><SearchableSelect ariaLabel="의뢰 선택" items={requests} kind="request" value={selectedRequestId} onChange={(value) => void onSelectRequest(value)} placeholder="의뢰 선택" /></label><label><span>하중 경우</span><SearchableSelect ariaLabel="하중 경우 선택" items={loadCases} kind="load_case" value={selectedLoadCaseId} disabled={loadCases.length === 0} onChange={(value) => void onSelectLoadCase(value)} placeholder={loadCases.length === 0 ? '미지정' : '하중 경우 선택'} /></label>{activeView !== 'workflow' && <ResultVersionSelector runs={analysisRuns} selectedRunId={selectedAnalysisRunId} loading={analysisRunsLoading} changing={analysisRunChanging} error={analysisRunError} onChange={onSelectAnalysisRun} />}</div>
    </section>
