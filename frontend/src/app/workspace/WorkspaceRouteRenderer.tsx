...
export function WorkspaceRouteRenderer({
  ...
  if (workspacePage === 'schemas') return <Suspense fallback={<FeatureScreenFallback />}><FolderSchemaWorkspace isVocabularyAdmin={hasPermission(authUser, 'system.catalog.manage', selectedProjectId)} canImportResults={hasPermission(authUser, 'result.import', selectedProjectId)} canImportForProject={(projectId) => hasPermission(authUser, 'result.import', projectId)} selectedProjectId={selectedProjectId} selectedRequestId={selectedRequestId} /></Suspense>
