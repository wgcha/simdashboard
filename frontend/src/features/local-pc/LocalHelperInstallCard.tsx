...
export function LocalHelperInstallCard({ busy = false }: { busy?: boolean }) {
  ...
  return <section className="local-pc-setup-card" aria-labelledby="local-pc-setup-heading">
    <div className="local-pc-card-heading"><span className="local-pc-eyebrow"><Download aria-hidden="true" /> FIRST-TIME SETUP</span><h2 id="local-pc-setup-heading">PC 도우미 설치</h2><p>이 PC에서 프로그램을 찾고 실행할 도우미를 설치하세요. 필요한 실행 환경이 함께 설치됩니다. 도우미는 선택 기능이며 대시보드 조회에는 필요하지 않습니다.</p></div>
