...
export function ResultsWorkspace(props: Props) {
  ...
  return <><div className="recommended-layout-controls">{canEdit && !editMode && dashboard.widgets.some((widget) => widget.type === 'video_grid') && <button type="button" onClick={props.onBeginEditing}>영상 배치 편집</button>}{canEdit && editMode && <button type="button" data-testid="recommended-layout-open" onClick={preview}>추천 배치 미리보기</button>}{canEdit && editMode && applied && <button type="button" data-testid="recommended-layout-restore" onClick={restore}>적용 전 배치로</button>}</div>{props.activeView !== 'compare' ? <SimulationDashboard projectId={props.overview.load_case.project_id} requestId={props.overview.load_case.request_id} /> : null}<ResultsWorkspaceGrid {...props} onLayoutChange={change} /><RecommendedLayoutPreview open={open} current={dashboard.widgets} suggestion={suggestion} stale={stale} onApply={apply} onClose={() => setOpen(false)} /></>
}
