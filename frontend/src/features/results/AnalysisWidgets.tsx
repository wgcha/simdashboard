...
function ChassisWidgetContent({ type, overview, threshold, canManageThreshold, onSaveThreshold, variableId, variableIds, hasExplicitSelection }: { type: DashboardWidget['type']; overview: Overview; threshold?: QualityThreshold; canManageThreshold: boolean; onSaveThreshold: (value: number) => void; variableId: string; variableIds?: string[]; hasExplicitSelection: boolean }) {
  ...
  if (type === 'chassis_bar') return <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} layout="vertical" margin={{top:8,right:24,left:8,bottom:4}}><CartesianGrid horizontal={false} stroke="var(--color-chart-grid)"/><XAxis type="number" domain={[0,Math.max(8,limit+2)]}/><YAxis type="category" dataKey="name" width={60} tick={{fill:'var(--color-chart-axis)',fontSize:10.8}}/><Tooltip formatter={(value:number)=>[`${value} mm`,'영구변형']}/><ReferenceLine x={limit} stroke="var(--color-warning)" strokeDasharray="5 4"/><Bar dataKey="value">{chartData.map((entry)=><Cell key={entry.name} fill={entry.verdict==='FAIL'?'var(--color-danger)':'var(--color-success)'}/>)}</Bar></BarChart></ResponsiveContainer>
