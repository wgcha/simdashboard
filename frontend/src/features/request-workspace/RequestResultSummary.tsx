...
export function RequestResultSummary({ activeView, comparisonAvailable, overview, onOpenComparison, onSelectEdges, runNo }: {
  ...
  const scalarResults = overview.scalar_results.filter((result) => result.result_group === resultGroup && (activeView !== 'open_cell' || result.unit.toLowerCase() === 'mpa') && (activeView !== 'open_cell' || result.variable_key.toLowerCase().includes('stress')))
  ...
  const verdictLabel = domainVerdict === 'NO_DATA' ? '결과 없음' : domainVerdict
  ...
  return <section className="request-result-summary" aria-label="현재 Run 핵심 결과">
    ...
    {primary ? <div className="result-summary-value"><span>{failing.length ? '초과 항목' : activeView === 'custom' ? '등록된 측정 항목' : '최대 측정값'}</span><strong>{primary.value_double.toFixed(2)} <small>{primary.unit}</small></strong><b>{primary.display_name}</b>{primaryEdge && failing.length > 0 && <button type="button" onClick={() => onSelectEdges([primaryEdge])}><AlertTriangle /> 해당 엣지 보기</button>}</div> : <div className="result-summary-value no-data"><span>측정 결과</span><strong>이 분석에 등록된 수치 없음</strong><b>결과를 가져온 뒤 판정합니다.</b></div>}
    <div className="result-summary-meta"><span>판정 기준</span><strong>{primary && Number.isFinite(primary.threshold_double) ? `${primary.threshold_double.toFixed(2)} ${primary.unit}` : '이 Run에 기록된 기준 없음'}</strong><b>{runNo != null ? `Run #${runNo}` : overview.run ? 'Run 확인 중' : 'Run 미등록'}</b></div>
