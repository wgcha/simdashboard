...
export function SemanticMappingWorkspace({ onLegacy, isVocabularyAdmin = false, selectedProjectId = '' }: { onLegacy: () => void; isVocabularyAdmin?: boolean; selectedProjectId?: string }) {
  ...
  useEffect(() => { previewAbort.current?.abort(); previewAbort.current = null; previewGeneration.current += 1; setPreview(null); setImpact(null); setImpactPayload(null); impactGeneration.current += 1; if (busy === 'preview') setBusy('') }, [recipe, template])
  ...
  const selectSample = async (file: File | null) => {
    ...
    try {
      ...
      const units = new Map((inspected.field_details ?? []).flatMap((detail) => detail.source && detail.unit ? [[detail.source, detail.unit] as const] : []))
      setRecipe((current) => recipeId ? current : ({ ...current, reader_version: suggestion?.reader_version ?? 2, input_layout: suggestion?.input_layout, format: inspected.format, delimiter: suggestion?.delimiter ?? current.delimiter, encoding: suggestion?.encoding ?? current.encoding, header_row: suggestion?.header_row ?? current.header_row, records_path: suggestion?.records_path ?? current.records_path, required_fields: [], mappings: fields.map((field) => ({ result_item_id: '', source: field, source_unit: units.get(field), dimensions: {}, missing: 'error', aggregate: 'none' })) }))
  ...
  const addMapping = () => setRecipe((current) => ({ ...current, mappings: [...current.mappings, { result_item_id: '', source: inspectionSources()[0] ?? '', dimensions: {}, missing: 'error', aggregate: 'none' }] }))
  ...
  const addMappings = (sources: string[]) => setRecipe((current) => {
    ...
    try {
      for (const source of sources) {
        ...
        try {
          ...
          const inferredType: SemanticItemDefinition['data_type'] = typeof sampleValue === 'boolean' ? 'BOOLEAN' : typeof sampleValue === 'number' ? (Number.isInteger(sampleValue) ? 'INTEGER' : 'FLOAT') : numericText ? (Number.isInteger(Number(sampleValue)) ? 'INTEGER' : 'FLOAT') : 'TEXT'
          const definition = { ...defaultItem(source), data_type: inferredType, unit: detail?.unit ?? '' }
          created.push({ source, item: await semanticMappingApi.createItem(definition) })
        } catch { failures.push(source) }
      }
      setCatalog((current) => ({ ...current, items: [...created.map(({ item }) => item), ...current.items] }))
      ...
      if (generation === sampleGeneration.current) setMessage({ kind: failures.length ? 'info' : 'success', text: `${created.length}개 결과 항목을 생성하고 샘플 필드에 연결했습니다.${failures.length ? ` 실패 ${failures.length}개는 그대로 남겨두었습니다.` : ''}` })
...
function RecipeTabContent(props: {
  sample: File | null; selectSample: (file: File | null) => void; resetSample: () => void; reinspectSample: () => void; onAddMappings: (sources: string[]) => void; onCreateItems: (sources: string[]) => void; onInspectionPage: (response: InspectResponse) => void; inspect: InspectResponse | null; recipe: SemanticRecipeDefinition; setRecipe: (recipe: SemanticRecipeDefinition) => void; catalog: SemanticCatalog; updateMapping: (index: number, patch: Partial<RecipeMapping>) => void; addMapping: () => void; removeMapping: (index: number) => void; createItem: (index: number) => void; busy: string; recipeName: string; setRecipeName: (name: string) => void; saveRecipe: () => void; activateBundle: () => void; template: SemanticTemplateDefinition; setTemplate: (template: SemanticTemplateDefinition) => void; templateName: string; setTemplateName: (name: string) => void; saveTemplate: () => void; preview: PreviewResponse | null; runPreview: () => void; recipeId: string; templateId: string; selectRecipe: (id: string) => void; selectTemplate: (id: string) => void; vocabularyScopeProjectId: string; setVocabularyScopeProjectId: (value: string) => void; scopeProjects: ContextProject[]
}) {
