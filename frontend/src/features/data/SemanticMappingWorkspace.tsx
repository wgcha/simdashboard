...
import { hasSampleValue, isNumericVectorSample, suggestedVectorComponents } from './semanticSampleValue'
import { compatibleItemMeaning, resolveAutomaticItem } from './semanticItemIdentity'
...
export function SemanticMappingWorkspace({ onLegacy, canImportResults = true, canImportForProject, isVocabularyAdmin = false, selectedProjectId = '' }: { onLegacy: () => void; canImportResults?: boolean; canImportForProject?: (projectId: string) => boolean; isVocabularyAdmin?: boolean; selectedProjectId?: string }) {
  const projectImportAllowed = (projectId: string) => canImportForProject ? canImportForProject(projectId) : canImportResults
  ...
  const [itemDialog, setItemDialog] = useState<{ mappingIndex: number; definition: SemanticItemDefinition; id?: string; expectedVersion?: number; automaticBaseKey?: string } | null>(null)
  ...
  const createItem = (mappingIndex: number) => {
    const source = recipe.mappings[mappingIndex]?.source ?? ''; setItemUsage(null)
    const detail = inspect?.field_details?.find((detail) => detail.source === source)
    const vector = isNumericVectorSample(detail?.value)
    const suggested: SemanticItemDefinition = { ...defaultItem(source), kind: vector ? 'vector' : 'scalar', unit: detail?.unit ?? '', ...(vector ? { components: suggestedVectorComponents(detail.value as unknown[], detail.label ?? sourceLabel(source)) } : {}) }
    const resolved = resolveAutomaticItem(suggested, catalog.items)
    const dialog = { mappingIndex, definition: resolved.definition, automaticBaseKey: suggested.key }
    setItemDialog(dialog); itemDialogRef.current = dialog
  }
  ...
  const saveItem = async (definition: SemanticItemDefinition) => {
    ...
    try {
      const automatic = !dialog.id && definition.key === dialog.definition.key
      const resolved = automatic ? resolveAutomaticItem({ ...definition, key: dialog.automaticBaseKey ?? definition.key }, catalog.items) : { definition, item: !dialog.id ? catalog.items.find((candidate) => candidate.lifecycle_status !== 'ARCHIVED' && compatibleItemMeaning(candidate.definition, definition)) : undefined }
      const item = resolved.item ?? await semanticMappingApi.saveItem(resolved.definition, dialog.id, dialog.expectedVersion)
      ...
      updateMapping(dialog.mappingIndex, { result_item_id: item.id, ...(recipe.reader_version === 2 && (item.definition.kind === 'vector' || !hasSampleValue(inspect?.field_details?.find((detail) => detail.source === recipe.mappings[dialog.mappingIndex]?.source)?.value)) ? { missing: 'preserve' as const } : {}) }); setItemDialog(null); itemDialogRef.current = null
      setMessage({ kind: 'success', text: `결과 항목 ${definition.label}을 저장했습니다. 항목 키: ${item.definition.key}` })
  ...
  const addSummaryTables = () => {
    const ids = [...new Set(recipe.mappings.map((mapping) => mapping.result_item_id).filter((id) => catalog.items.some((item) => item.id === id && ['scalar', 'vector'].includes(item.definition.kind) && item.lifecycle_status !== 'ARCHIVED')))]
    if (!ids.length) { setMessage({ kind: 'info', text: '요약표에 연결할 단일값·벡터 항목을 먼저 정의하세요.' }); return }
    ...
    setMessage({ kind: 'success', text: `${ids.length}개 단일값·벡터를 요약표 ${count}개에 연결했습니다. 곡선은 선 그래프로 추가하세요.` })
  }
  ...
  const bundlePayload = () => {
    ...
    if (!preview || preview.widgets.some((widget) => !['READY', 'NO_VALUE'].includes(widget.status))) { setMessage({ kind: 'error', text: '샘플 미리보기에서 모든 위젯이 정상 또는 정의된 값 없음인지 확인한 뒤 활성화하세요.' }); return }
  ...
  const addMappings = (sources: string[]) => { configurationGeneration.current += 1; setRecipe((current) => {
    ...
    const additions = sources.filter((source) => { const detail = inspect?.field_details?.find((candidate) => candidate.source === source); return source && !existing.has(source) && !!detail && detail.mappable !== false && (detail.value == null || typeof detail.value !== 'object' || isNumericVectorSample(detail.value)) }).map((source) => ({ result_item_id: '', source, dimensions: {}, missing: recipe.reader_version === 2 ? 'preserve' as const : 'error' as const, aggregate: 'none' as const }))
    ...
    try {
      for (const source of sources) {
        ...
        try {
          ...
          const vector = isNumericVectorSample(sampleValue)
          const definition: SemanticItemDefinition = { ...defaultItem(source), kind: vector ? 'vector' : 'scalar', data_type: vector || !hasSampleValue(sampleValue) ? 'FLOAT' : inferredType, unit: detail?.unit ?? '', ...(vector ? { components: suggestedVectorComponents(sampleValue, detail?.label ?? sourceLabel(source)) } : {}) }
          if (detail?.mappable === false || sampleValue != null && typeof sampleValue === 'object' && !isNumericVectorSample(sampleValue)) continue
          const resolved = resolveAutomaticItem(definition, [...catalog.items, ...created.map((entry) => entry.item)])
          created.push({ source, item: resolved.item ?? await semanticMappingApi.createItem(resolved.definition) })
      ...
      if (generation === sampleGeneration.current) setRecipe((current) => {
        ...
        for (const { source, item } of created) {
          ...
          if (blankIndex >= 0) { used.add(blankIndex); mappings[blankIndex] = { ...mappings[blankIndex], result_item_id: item.id, missing: recipe.reader_version === 2 ? 'preserve' : mappings[blankIndex].missing, source_unit: item.definition.unit || mappings[blankIndex].source_unit }; continue }
          if (!mappings.some((mapping) => mapping.source === source && mapping.result_item_id === item.id)) additions.push({ result_item_id: item.id, source, source_unit: item.definition.unit || undefined, dimensions: {}, missing: recipe.reader_version === 2 ? 'preserve' : 'error', aggregate: 'none' })
        }
...
function PreviewPanel({ parsed, widgets }: { parsed: ParsedSemanticResult; widgets: PreviewWidget[] }) {
  ...
  const countKind = (kind: string, fallback: number) => {
    if (observations.length) return observations.filter((row) => row.kind === kind).length
    const value = parsed.summary?.[`${kind}_count`]
    return typeof value === 'number' && Number.isFinite(value) && value >= 0 ? value : fallback
  }
  const scalarCount = countKind('scalar', parsed.scalars?.length ?? 0)
  const vectorCount = countKind('vector', parsed.vectors?.length ?? 0)
  const curveCount = countKind('curve', parsed.curves?.length ?? 0)
  const sampleRows = observations.slice(page * 50, (page + 1) * 50)

  return <div className="preview-panel"><div className="parsed-summary"><span><b>{scalarCount}</b> scalar</span><span><b>{vectorCount}</b> vector</span><span><b>{curveCount}</b> curve</span><span><b>{parsed.media?.length ?? 0}</b> media</span><span><b>{parsed.observations?.length ?? 0}</b> observations</span></div>{parsed.warnings?.length ? <div className="warning-list">{parsed.warnings.map((warning) => <span key={warning}><AlertTriangle />{warning}</span>)}</div> : null}<div className="sample-row-table-wrap"><table className="sample-row-table"><thead><tr><th>결과 항목</th><th>정규화 값</th><th>단위</th></tr></thead><tbody>{sampleRows.map((row, index) => <tr key={index}><td>{String(row.label ?? row.item_id ?? '')}</td><td>{row.value_status === 'MISSING' ? '값 없음' : Array.isArray(row.points) ? `${row.points.length}개 곡선 점` : Array.isArray(row.value) ? row.value.map((value, index) => `${Array.isArray(row.components) ? row.components[index] : `성분 ${index + 1}`}: ${value == null ? '—' : String(value)}`).join(' · ') : String(row.value ?? '')}</td><td>{String(row.unit ?? '')}</td></tr>)}</tbody></table></div><div className="sample-page-actions"><button className="ghost-button" disabled={page === 0} onClick={() => setPage(page - 1)}>이전 결과</button><span>{observations.length}개 결과</span><button className="ghost-button" disabled={(page + 1) * 50 >= observations.length} onClick={() => setPage(page + 1)}>다음 결과</button></div><SemanticWidgetGrid widgets={widgets} /></div>
}
