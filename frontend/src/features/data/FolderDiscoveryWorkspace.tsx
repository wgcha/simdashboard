...
export function FolderDiscoveryWorkspace({ onComplete, onOpenFolder }: { onComplete?: () => void; onOpenFolder?: (target?: FolderDiscoveryTarget) => void }) {
  ...
  const invalidatePreview = useCallback(() => { setPreview(null); setAppliedPreviewId('') }, [])
  ...
  const folderTarget = (row: FolderDiscoveryPreview['rows'][number]): FolderDiscoveryTarget | undefined => row.role_kind === 'RESULTS' || row.role_kind === 'INPUT' ? { relative_path: row.relative_path, project_id: row.project_id, request_id: row.request_id, load_case_id: row.load_case_id, role: row.role_kind } : undefined
...
function RuleEditor({ rule, roleOptions, analysisTypes, catalog, onChange, onDelete }: { rule: FolderDiscoveryRule; roleOptions: FolderRoleOption[]; analysisTypes: { key: string; label: string }[]; catalog: FolderDiscoveryCatalog | null; onChange: (patch: Partial<FolderDiscoveryRule>) => void; onDelete: () => void }) {
  ...
  return <div className="folder-discovery-rule">
    ...
    <label>구분자<input aria-label={`${roleName(rule.role, catalog)} 구분자`} value={rule.delimiter ?? ''} maxLength={8} placeholder="없음" onChange={(event) => onChange({ delimiter: event.target.value })} /><small>비워 두면 전체 이름·빈 코드</small></label>
