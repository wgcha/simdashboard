...
import { semanticSampleValue } from './semanticSampleValue'
...
const suggestedType = (item: Item): SemanticWidgetType => item.definition.kind === 'curve' ? 'line' : item.definition.kind === 'scalar' ? 'kpi' : item.definition.kind
...
export function SemanticWidgetEditor({ widget, items, mappings = [], inspect, readerVersion, onChange, onRemove }: Props) {
  ...
  const changeItems = (nextIds: string[]) => {
    ...
    const type = next.length && !next.every((item) => compatible(widget.type, item)) ? suggestedType(next[0]) : widget.type
    ...
    onChange({ ...widget, type, item_ids: actual.map((item) => item.id),
      ...
      filters: {}, display_unit: undefined, x_display_unit: undefined, y_display_unit: undefined, threshold: undefined })
  }
  ...
  return <section className="widget-editor semantic-widget-editor" aria-label={`위젯 설정 ${widget.title}`}>
    ...
    <p className="semantic-widget-help">{widget.type === 'line' ? 'X/Y 필드가 연결된 곡선 항목만 선택할 수 있습니다.' : widget.type === 'scatter' ? '같은 측정 위치의 X/Y 값을 짝지어 표시합니다. 한 쌍의 값은 한 점으로 표시됩니다.' : '하나의 항목에 하나의 값은 값 카드로 표시합니다. 좌표 성분은 표나 각각의 카드로 구성합니다.'}</p>
    ...
    {sampleDetails.length ? <details className="semantic-widget-source-preview"><summary>연결된 원본 샘플 {sampleDetails.length}개 보기</summary><p>아래는 변환 전 원본입니다. 적용 결과는 통합 미리보기에서 확인합니다.</p><ul>{sampleDetails.map((field) => <li key={field.source}><span>{field.label ?? field.source}</span><strong>{field.value === null || field.value === undefined ? '값 없음' : String(field.value)} {field.unit ?? ''}</strong></li>)}</ul></details> : null}
  </section>
}
