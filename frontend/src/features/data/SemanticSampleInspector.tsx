...
export function SemanticSampleInspector({ sample, inspect, busy, onSelectSample, onReset, onAddMappings, onCreateItems, onInspectionPage }: InspectorProps) {
  ...
  const toggleVisible = () => setSelected((current) => { const next = new Set(current); const all = visibleSources.every((source) => next.has(source)); visibleSources.forEach((source) => all ? next.delete(source) : next.add(source)); return next })
