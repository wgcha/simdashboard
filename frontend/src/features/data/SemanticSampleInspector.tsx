...
function safeValue(value: unknown) {
  if (value === null) return <span className="sample-value null">null</span>
  if (value === undefined) return <span className="sample-value missing">—</span>
  if (typeof value === 'string' && value.length === 0) return <span className="sample-value blank">(blank)</span>
  ...
  if (Array.isArray(value) || (typeof value === 'object' && value !== null)) return <details className="sample-value-object"><summary><ChevronDown /> {Array.isArray(value) ? `array · ${value.length}` : 'object'}</summary><code>{JSON.stringify(value, null, 2)}</code></details>
...
export function SemanticSampleInspector({ sample, inspect, busy, onSelectSample, onReset, onAddMappings, onCreateItems, onInspectionPage }: InspectorProps) {
  ...
  useEffect(() => {
    ...
    if (!inspect?.upload_id || selectionUpload.current !== inspect.upload_id) {
      setSelected(new Set(detailFor(inspect).filter((detail) => detail.value != null && detail.value !== '' && detail.mappable !== false && !Array.isArray(detail.value) && (typeof detail.value !== 'object')).map(fieldName).filter(Boolean)))
    }
  ...
  const visibleDetails = details.slice(fieldOffset, fieldOffset + PAGE_FIELDS)
  const visibleSources = visibleDetails.map(fieldName).filter(Boolean)
  const selectableSources = visibleDetails.filter((detail) => detail.value != null && detail.value !== '' && detail.mappable !== false && !Array.isArray(detail.value)).map(fieldName).filter(Boolean)
  const toggle = (source: string) => setSelected((current) => { const next = new Set(current); if (next.has(source)) next.delete(source); else next.add(source); return next })
  ...
  const addSelected = () => { const sources = [...selected]; if (sources.length) { onAddMappings?.(sources); setSelected(new Set()) } }
