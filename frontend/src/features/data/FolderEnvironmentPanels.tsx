...
import { folderEnvironmentApi, type FolderEnvironment, type FolderEnvironmentProfile, type FolderEnvironmentProfileRule, type FolderEnvironmentRegistration } from '../../shared/api/folderEnvironment'
...
export function FolderProfileEditor({ environment, profiles, seedRules, onSaved, onNotice }: { environment: FolderEnvironment; profiles: FolderEnvironmentProfile[]; seedRules: FolderEnvironmentProfileRule[]; onSaved: (profile: FolderEnvironmentProfile) => void; onNotice: (message: string) => void }) {
  ...
  useEffect(() => { setEditing(''); setName(''); setRules([]) }, [environment])
  ...
  const save = async (copy = false) => {
    setSaving(true)
    try {
      const body = { environment, name, rules: { rules } }
  ...
  return <article className="folder-environment-card">
    <h2>저장된 규칙</h2><div className="folder-environment-form"><label>편집할 규칙<select value={editing} disabled={saving} onChange={(event) => { const item = profiles.find((p) => p.id === event.target.value); setEditing(item?.id ?? ''); setName(item?.name ?? ''); setRules(item?.rules.rules ?? []) }}><option value="">새 규칙</option>{profiles.map((p) => <option value={p.id} key={p.id}>{p.name} · v{p.revision}</option>)}</select></label><label>규칙 이름<input value={name} onChange={(event) => setName(event.target.value)} disabled={saving} /></label></div>
    <div className="folder-environment-footer"><span>기본 환경 인식 위에 아래 규칙을 적용합니다. 여러 역할이 겹치면 확인 대상으로 남깁니다.</span><button type="button" className="ghost-button" disabled={saving || !seedRules.length} onClick={() => { setEditing(''); setRules(seedRules); setName('확인한 폴더 규칙') }}>확인한 구조에서 가져오기</button><button type="button" className="ghost-button" disabled={saving} onClick={() => setRules([...rules, { role_kind: 'SIMULATION_CASE', parent_role: 'REQUEST', pattern: '', match_mode: 'glob' }])}>규칙 추가</button></div>
