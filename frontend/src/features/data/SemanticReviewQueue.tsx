...
import { semanticStatusLabel } from '../../shared/components/semanticLabels'
...
export function SemanticReviewQueue({ binding, catalog, canReview, refreshToken = 0, onMessage }: Props) {
  ...
  return <section className="semantic-card semantic-review-queue" aria-labelledby="semantic-review-heading">
    ...
    <div className="semantic-review-items">{items.map((item) => <article className={`semantic-review-item ${selectedId === item.id ? 'selected' : ''}`} key={item.id}>
      <button type="button" className="semantic-review-item-header" onClick={() => chooseItem(item)} aria-expanded={selectedId === item.id}><i>{statusIcon(item.review_state)}</i><strong>{item.relative_path}</strong><b>{semanticStatusLabel(item.review_state)}</b></button>
      <div className="semantic-review-item-meta"><span><em>판별</em>{semanticStatusLabel(item.scan_status)}</span><span><em>크기</em>{formatSize(item.source_size ?? null)}</span><span><em>개정</em>{item.revision}</span><span><em>후보</em>{item.candidates?.length ?? 0}개</span></div>
