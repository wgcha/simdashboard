...
import type { NameValueChartStyle, RequestResultDefinition, RequestResultWidget, WorkbenchTaskType } from './types'
...
export function RequestResultWidgetConfiguration({ value, tasks, onChange }: { value: RequestResultDefinition | null; tasks: WorkbenchTaskType[]; onChange: (value: RequestResultDefinition | null) => void }) {
  ...
  const addWidget = (type: WidgetType) => {
    ...
    onChange({ ...definition, widgets: [...definition.widgets, { id: widgetIdForType(type), type, title: catalog.label, variable_key: catalog.variable_key ?? null, data_contracts: normalizeResultDataContracts(catalog.data_contracts), chart_style: type === 'name_value' ? 'bar' : undefined, required: false }] })
  }
