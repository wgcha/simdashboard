...
import type { AnalysisTemplateVersion, ResultProfile, WorkbenchTaskType } from './types'
...
export function ResultProfileConfiguration({ templates, value, availableDataContracts, validationTasks, onChange }: {
  templates: AnalysisTemplateVersion[]
  value: ResultProfile | null
  availableDataContracts: string[]
  validationTasks: WorkbenchTaskType[]
  ...
  const availableContracts = new Set(normalizeResultDataContracts(availableDataContracts))
  const validationError = resultProfileValidation(value, validationTasks)
  ...
  return <section className="result-profile-configuration">
    ...
    {selectedTemplate && <><fieldset className="result-profile-contracts"><legend>결과 표시 전 필요한 데이터 계약</legend>{RESULT_DATA_CONTRACTS.map((contract) => <label key={contract}><input type="checkbox" checked={requiredDataContracts.includes(contract) ?? false} disabled={!availableContracts.has(contract)} onChange={(event) => update({ required_data_contracts: event.target.checked ? normalizeResultDataContracts([...requiredDataContracts, contract]) : requiredDataContracts.filter((item) => item !== contract) })} />{contract}</label>)}</fieldset>
