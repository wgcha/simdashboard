...
const COLORS = ['var(--color-chart-series-1)', 'var(--color-success)', 'var(--color-warning)', 'var(--color-danger)', 'var(--color-chart-series-5)']
...
export function PortfolioDashboard({ refreshToken, editMode, layout, layoutVersion, onLayoutChange, onCancelEdit, onResetLayout, onOpen, onOpenResult }: { refreshToken: number; editMode: boolean; layout: PortfolioLayout; layoutVersion: number; onLayoutChange: (layout: PortfolioLayout) => void; onCancelEdit: () => void; onResetLayout: () => void; onOpen: (projectId: string, requestId: string, loadCaseId: string) => void; onOpenResult?: (projectId: string, requestId: string, loadCaseId: string, runId: string) => void }) {
  ...
  const chartCards: Record<string, ReactNode> = {
    trend: <ChartCard title="의뢰·완료·실패 추이" subtitle="의뢰 접수일 기준"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.trend}><CartesianGrid stroke="var(--color-chart-grid)" vertical={false} /><XAxis dataKey="date" tick={{ fill:'var(--color-chart-axis)', fontSize:chartFontSize }} /><YAxis allowDecimals={false} tick={{ fill:'var(--color-chart-axis)', fontSize:chartFontSize }} /><Tooltip /><Legend /><Area dataKey="requests" name="하중 경우" stroke="var(--color-chart-series-1)" fill="var(--color-chart-series-1)" fillOpacity={0.14} /><Area dataKey="completed" name="결과 보유" stroke="var(--color-success)" fill="var(--color-success)" fillOpacity={0.14} /><Area dataKey="failed" name="FAIL" stroke="var(--color-danger)" fill="var(--color-danger)" fillOpacity={0.14} /></AreaChart></ResponsiveContainer></ChartCard>,
