...
import { AlertTriangle, BarChart3, CheckCircle2, Database, Download, LoaderCircle, Search, SlidersHorizontal } from 'lucide-react'
...
export function PortfolioDashboard({ onOpen }: { onOpen: (projectId: string, requestId: string, loadCaseId: string) => void }) {
  ...
  return <div className="portfolio-page">
    ...
    {data.records.length === 0 ? <div className="portfolio-empty"><Database /><h2>조건에 맞는 해석 데이터가 없습니다.</h2><p>기간이나 분류 필터를 넓혀 보세요.</p><button onClick={reset}>전체 데이터 보기</button></div> : <>
      <section className="portfolio-grid">
        <ChartCard title="의뢰·완료·실패 추이" subtitle="의뢰 접수일 기준"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.trend}><CartesianGrid stroke="#20394d" vertical={false} /><XAxis dataKey="date" tick={{ fill:'#6f899b', fontSize:10 }} /><YAxis allowDecimals={false} tick={{ fill:'#6f899b', fontSize:10 }} /><Tooltip /><Legend /><Area dataKey="requests" name="하중 경우" stroke="#50d5ff" fill="#50d5ff22" /><Area dataKey="completed" name="결과 보유" stroke="#70e0a8" fill="#70e0a822" /><Area dataKey="failed" name="FAIL" stroke="#ff647d" fill="#ff647d22" /></AreaChart></ResponsiveContainer></ChartCard>
        <ChartCard title="의뢰 상태 분포" subtitle="현재 상태"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data.status_distribution} dataKey="value" nameKey="name" innerRadius="52%" outerRadius="76%" paddingAngle={3}>{data.status_distribution.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip formatter={(v, n) => [v, STATUS_LABEL[String(n)] ?? n]} /><Legend formatter={(v) => STATUS_LABEL[String(v)] ?? v} /></PieChart></ResponsiveContainer></ChartCard>
        <ChartCard title="해석 유형별 품질" subtitle="최신 실행 판정"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.quality_by_type}><CartesianGrid stroke="#20394d" vertical={false} /><XAxis dataKey="type" tick={{ fill:'#7892a4', fontSize:10 }} /><YAxis allowDecimals={false} /><Tooltip /><Legend /><Bar dataKey="pass" name="PASS" stackId="a" fill="#70e0a8" /><Bar dataKey="fail" name="FAIL" stackId="a" fill="#ff647d" /><Bar dataKey="no_data" name="NO DATA" stackId="a" fill="#526c7e" /></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="해석 유형 구성" subtitle="필터 적용 결과"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.type_distribution} layout="vertical"><CartesianGrid stroke="#20394d" horizontal={false} /><XAxis type="number" allowDecimals={false} /><YAxis dataKey="name" type="category" width={90} tick={{ fill:'#7892a4', fontSize:10 }} /><Tooltip /><Bar dataKey="value" name="하중 경우" fill="#50d5ff" radius={[0,5,5,0]} /></BarChart></ResponsiveContainer></ChartCard>
      </section>
