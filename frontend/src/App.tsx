...
import type { AnalysisRequest, AutomationTemplate, DashboardDefinition, DashboardSummary, DashboardVersion, DashboardWidget, LoadCase, Overview, Project, QualityThreshold, VariableDefinition, WidgetCatalogItem, Workflow, WorkflowStep } from './types'
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-shell">
        ...
        {workspacePage === 'portfolio' ? <PortfolioDashboard onOpen={(projectId, requestId, loadCaseId) => void openImportedResult(projectId, requestId, loadCaseId)} /> : workspacePage === 'variables' ? <VariableCatalogPage variables={variables} overview={overview} /> : workspacePage === 'templates' ? <AutomationTemplatesPage /> : workspacePage === 'data' ? (
...
function WidgetContent({ widget, overview, selectedEdges, threshold, onSaveThreshold }: { widget: DashboardWidget; overview: Overview; selectedEdges: string[]; threshold?: QualityThreshold; onSaveThreshold: (value: number) => void }) {
  ...
  if (type.startsWith('chassis_')) return <ChassisWidgetContent type={type} overview={overview} threshold={threshold} onSaveThreshold={onSaveThreshold} />
...
function ChassisWidgetContent({ type, overview, threshold, onSaveThreshold }: { type: DashboardWidget['type']; overview: Overview; threshold?: QualityThreshold; onSaveThreshold: (value: number) => void }) {
