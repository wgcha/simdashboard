...
import { refreshCurrentLoadCaseResults as runCurrentLoadCaseResults } from './features/results/refreshCurrentLoadCaseResults'
...
function App() {
  ...
  const handleAnalysisRunChange = async (runId: string) => {
    ...
    try { return await selectAnalysisRun(runId) } finally { workspaceContextChangePending.current = false }
  }
  const refreshCurrentLoadCaseResults = async () => {
    void runCurrentLoadCaseResults({ context: { projectId: selectedProjectId, requestId: selectedRequestId, loadCaseId: selectedLoadCaseId }, activeView, workspacePage, canImport: hasPermission(authUser, 'result.import', selectedProjectId), beginIntent: beginContextEntry, isCurrentIntent: isCurrentContextEntry, currentContext: () => selectedContextRef.current, selectRun: handleAnalysisRunChange, openReview: openImportedResult, setBusy: setResultsRefreshing, setNotice, setError })
  }
  ...
  if (workspacePage !== 'access_admin' && workspacePage !== 'schemas' && (!overview || !dashboard) && !caseResultsMode && !isRequestMonitoring && !pendingAnalysis && !(projects.length > 0 && !isWorkspaceIndex && (workspacePage === 'portfolio' || workspacePage === 'workbench' || workspacePage === 'data' || workspacePage === 'intake'))) {
    ...
    return <BootstrapWorkspaceShell
      ...
      canOpenIntake={projects.length > 0 && canCreateRequest} canOpenFolders={Boolean(authUser?.is_global_admin) && hasPermission(authUser, 'system.catalog.manage', selectedProjectId)}
  ...
  // The version hook resets asynchronously on a load-case change. Read the Run only
  ...
  return (
    ...
    >
      <AppShellMain topbar={<AppTopbar breadcrumb={breadcrumb} actions={<>
        ...
        {semanticReviewPanel}
        {(pendingAnalysis || (activeView !== 'workflow' && !dashboard)) ? <Suspense fallback={<FeatureScreenFallback />}><PendingAnalysisWorkspace projectId={selectedProjectId} projectName={analysisProjectName} requestId={selectedRequestId} requestTitle={analysisTitle} selectedLoadCaseId={selectedLoadCaseId} selectedRunId={reviewRunId} canOpenData={hasPermission(authUser, 'result.import', selectedProjectId)} onOpenData={() => enterWorkspace('data')} onLoadLayout={workbenchApi.requestResultLayout} onSnapshotPageChange={hydrateSnapshotDashboard} /></Suspense> : activeView !== 'workflow' ? <Suspense fallback={<FeatureScreenFallback />}><ResultsWorkspace
