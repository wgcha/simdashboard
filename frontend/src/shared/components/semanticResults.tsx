...
function ResultTable({ data }: { data: unknown[] }) {
  ...
  const cell = (row: Observation, key: string) => key === 'dimensions' && row[key] ? JSON.stringify(row[key]) : scalarValue(row[key])
