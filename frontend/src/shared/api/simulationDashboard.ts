...
export type DashboardChoice = { id: string; label: string; disabled?: boolean; reason?: string | null; case_id?: string | null; load_case_id?: string | null; execution_run_id?: string | null; run_id?: string | null; mode?: string | null; capture_id?: string | null; component_id?: string | null }
...
export type DashboardValue = { verdict?: string | null; value: number | null; unit: string | null; unit_status?: string | null; completeness?: string | null; status?: string | null; basis?: string | null; scope?: string | null }
export type DashboardContext = { project_id: string; request_id: string; simulation_case_id?: string; load_case_id?: string; execution_run_id?: string; mode?: string; capture_id?: string; component_id?: string; basis?: DashboardBasis; scene_id?: string }
export type DashboardContextEcho = DashboardContext & { context_key: string }
export type DashboardCatalog = { environment: DashboardEnvironment; cases: DashboardChoice[]; load_cases: DashboardChoice[]; execution_runs: DashboardChoice[]; modes: DashboardChoice[]; captures: DashboardChoice[]; components: DashboardChoice[]; bases: DashboardChoice[] }
...
export type DashboardMember = { id: string; label: string; simulation_case_id: string; design_description?: string | null; color?: string | null; pattern?: string |null; load_case_id: string; execution_run_id: string; mode: string; capture_id: string; component_id: string; basis: DashboardBasis }
...
export type DashboardComparisonMember = { simulation_case_id: string; load_case_id: string; execution_run_id: string; capture_id: string; mode: string; component_id: string; basis: DashboardBasis }
...
export const simulationDashboardApi = {
  ...
  distribution: (runId: string, context: Pick<DashboardContext, 'capture_id' | 'mode' | 'component_id' | 'basis'> & { edge_keys?: string; line_indices?: string }, signal?: AbortSignal) => read<DashboardDistribution>(dashboardPath('distribution', 'runs', encodeURIComponent(runId)), { capture_id: context.capture_id, mode: context.mode, component_id: context.component_id, basis: context.basis, edge_keys: context.edge_keys, line_indices: context.line_indices }, signal),
  ...
  sceneDetail: (sceneId: string, context: Pick<DashboardContext, 'execution_run_id' | 'capture_id' | 'mode' | 'component_id' | 'basis'> & { line_indices?: string; position?: string }, signal?: AbortSignal) => read<DashboardSceneDetail>(dashboardPath('distribution', 'scenes', encodeURIComponent(sceneId)), { run_id: context.execution_run_id, capture_id: context.capture_id, mode: context.mode, component_id: context.component_id, basis: context.basis, line_indices: context.line_indices, position: context.position }, signal),
