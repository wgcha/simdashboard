...
export type SemanticWidgetType = 'kpi' | 'gauge' | 'table' | 'bar' | 'name_value' | 'line' | 'scatter' | 'image' | 'video'
export type SemanticChartStyle = 'bar' | 'dot' | 'line'
export type SemanticWidgetDefinition = { id: string; type: SemanticWidgetType; title: string; item_ids: string[]; x_item_id?: string; y_item_id?: string; filters: Record<string, string>; decimals: number; display_unit?: string; threshold?: number; vector_component?: string; x_display_unit?: string; y_display_unit?: string; chart_style?: SemanticChartStyle }
