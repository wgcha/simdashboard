...
const restoreSource = readFileSync(new URL('../src/app/routing/useWorkspaceContextRestore.ts', import.meta.url), 'utf8')
const reviewHeaderSource = readFileSync(new URL('../src/app/workspace/RequestWorkspaceShellHeader.tsx', import.meta.url), 'utf8')
const genericLayoutSource = readFileSync(new URL('../src/features/results/GenericResultLayoutWorkspace.tsx', import.meta.url), 'utf8')
const resultLayoutApiSource = readFileSync(new URL('../src/shared/api/resultLayouts.ts', import.meta.url), 'utf8')
if (!appSource.includes("onReviewSnapshot: () =>") || !appSource.includes("onReviewUnconfigured: () =>") || !appSource.includes('const canEdit = projectSetup ? false : pendingAnalysis ? canDashboardEdit') || !appSource.includes("workspacePage === 'dashboard' && activeView !== 'workflow' && <button") || !appSource.includes("activeDashboardId === 'request-result-layout' ? !selectedLoadCaseId : !dashboardReady")) throw new Error('UNCONFIGURED result actions must remain visible with report disabled and edit actions enabled for a selected load case')
if (!appSource.includes("runId: activeView !== 'workflow' ? selectedAnalysisRunId || undefined : undefined") || !appSource.includes("const reviewRunId = overview?.load_case.id === selectedLoadCaseId ? overview.run ?? undefined : undefined") || !appSource.includes("selectedRunId={reviewRunId}") || !appSource.includes("runId: selectedAnalysisRunId || undefined, view: 'custom'") || !appSource.includes("clearSnapshotDashboard()")) throw new Error('virtual review layouts must preserve the selected Run, mount semantic widgets once, and reject stale cross-load-case requests')
if (restoreSource.includes('virtualResultLayout && context.runId')) throw new Error('a virtual review URL must accept a validated exact Run')
if (!reviewHeaderSource.includes("const showRunSelector = activeTab === 'review'")) throw new Error('snapshot and unconfigured review layouts must expose the selected Run control')
if (!genericLayoutSource.includes("selectedRunId || 'latest'") || !genericLayoutSource.includes('loaderRef.current(requestId, selectedLoadCaseId || undefined, selectedRunId)')) throw new Error('generic review layouts must cache and load bindings by the selected Run')
if (!resultLayoutApiSource.includes('requestResultLayoutQuery(loadCaseId?: string, runId?: string)') || !resultLayoutApiSource.includes("run_id: runId") || !resultLayoutApiSource.includes('requestResultLayoutQuery(loadCaseId, runId)')) throw new Error('result-layout queries must include an optional exact Run without inventing context')
