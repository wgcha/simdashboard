...
def test_voc_posts_are_server_attributed_paginated_and_exportable() -> None:
...
def test_voc_requires_an_active_account_and_reads_current_author_identity(monkeypatch) -> None:
    ...
    def insert_user(user_id: str, username: str, display_name: str, status: str, password: str | None) -> None:
        with connect() as connection:
            connection.execute(
                ...
                """,
                [user_id, username, hash_password(password) if password else None, display_name, now, now, status],
            )
    ...
    with TestClient(app) as client:
        ...
        assert client.get("/api/voc/export?format=json", headers=active_headers).status_code == 200
        ...
        for user_id, username, display_name, status in (
            ...
            assert client.get("/api/voc/export?format=json", headers=headers).status_code == 403
