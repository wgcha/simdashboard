...
def main() -> None:
    ...
    if profile not in {"local", "windows-vm-intranet", "windows-password-intranet", "windows-server-intranet-http", "rocky8", "rocky8-dev-http"}:
    ...
    if profile == "windows-server-intranet-http":
        failures = []
        if database.backend != "postgresql":
            failures.append("POSTGRESQL_REQUIRED")
        if security.auth_mode != "password":
        ...
        if failures:
            raise RuntimeError("WINDOWS_SERVER_HTTP_PROFILE_INVALID:" + ",".join(failures))
    if profile == "rocky8":
        failures = []
        if database.backend != "postgresql":
            failures.append("POSTGRESQL_REQUIRED")
