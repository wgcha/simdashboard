...
_SVG_ACTIVE_CONTENT = re.compile(
    rb"<(?:script|iframe|object|embed|foreignObject)\b|\bon[a-z][a-z0-9_-]*\s*=|"
    rb"(?:href|xlink:href)\s*=\s*['\"]\s*(?:https?:|//|javascript:|data:)|@import\b|"
    rb"url\s*\(\s*['\"]?\s*(?:https?:|//|javascript:|data:)",
    re.IGNORECASE,
)
...
def _has_mp4_ftyp_box(prefix: bytes) -> bool:
    """Validate top-level ISO BMFF boxes until an ftyp box is found."""
    offset = 0
    limit = min(len(prefix), MAX_PREFIX_BYTES)
    while offset + 8 <= limit:
        ...
        if size < header_size:
            return False
        if box_type == b"ftyp":
            return size >= header_size + 8 and offset + header_size + 8 <= limit
        offset += size
    return False
...
def _validate_magic(asset_type: str, filename: str, mime_type: str, prefix: bytes, file_size: int) -> None:
    ...
    if suffix == ".mp4" and not _has_mp4_ftyp_box(prefix):
...
def inspect_stream(
    ...
    *,
    ...
    """Validate and hash a media stream with bounded memory usage."""
    ...
    return MediaInspection(total, digest.hexdigest(), canonical_type)


def inspect_file(path: Path, *, filename: str | None = None, mime_type: str, asset_type: str) -> MediaInspection:
